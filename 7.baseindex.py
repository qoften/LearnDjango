...
<p>
    <a href="{% url 'logs:index' %}">Learning Log</a> # 创建段落，使用模板标签创建链接，段落包含项目名
</p>

{% block content %}{% endblock content %} # 一对块标签，名为content，作为占位符，信息由子模块指定

模版标签 用｛% %｝表示，{% url 'logs:index' %} 中，生成一个url与app1中views.py指向的urls.py定义URL模式匹配
...
｛% extends "logs/base.html" %｝ # 原标题替换从父模板继承的代码，让Django知道继承了哪个父模板
